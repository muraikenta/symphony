...
- **新しい Linear コメントを検知して自動で Todo に戻す IssueCommentMonitor**: 自分で作成した PR には `Request changes` できないので（GitHub の制約）、補完として Linear の issue コメントもポーリング。`Human PR Review` 中の issue に新規コメントが付くと `Todo` に自動遷移。Codex Workpad コメント、bot サマリー（`🐶 みらいいぬ自動調査` 等）、スレッド返信、Symphony 自身が投稿した cue コメントは対象外。issue 初回観測時はその時点の最新コメントをベースラインとして記録し、以降に追加された人間コメントだけがトリガー
- **任意ステートで Symphony と会話できる `conversational_states`**: `tracker.conversational_states`（デフォルト `["QA"]`）に列挙したステートに対しては、新コメント検知時に **会話モード用の cue コメント**（`🐧 Symphony cue: 会話モード`）を投稿してから `Todo` に遷移。エージェントは cue を読んで「コード変更せず、回答後に元のステートに戻す」conversation-only モードで動く。Human PR Review はこれまで通り feedback ループ。利用者が QA や Blocked のチケットで質問したい場合、issue にコメントするだけでエージェントが答えてくれる
- **スレッド返信もトリガー対象**: Linear の親コメントへのスレッド返信、および GitHub PR のインラインレビュー返信どちらも検知してエージェント起動するようになった。以前は `parent_id` ありのコメントを `IssueCommentMonitor` が除外していたため反応しなかった。
- **マージ済み PR でも会話モードを発火**: `PrReviewMonitor` が `conversational_states`（QA など）の issue を見るときは `gh pr list --state all` で merged/closed PR も対象。post-merge の PR スレッドへのコメントも検知して cue + Todo 経路で会話モードを起動。
