...
- **任意ステートで Symphony と会話できる `conversational_states`**: `tracker.conversational_states`（デフォルト `["QA"]`）に列挙したステートに対しては、新コメント検知時に Orchestrator の `request_dispatch/2` API を直接叩いてエージェントを spawn。**ステート移動も cue コメントも不要** — エージェントは `issue.state` をそのまま見て、conversational_states ならコード変更せず回答だけして turn 終了。利用者が QA や Blocked のチケットで質問するだけでエージェントが答え、ノイズコメントが残らない
